...
function displayComputers(category) {
    ...
    function updateBasketBadge() {
        ...
        const totalQuantity = addedItemsToBasket.reduce((total, item) => total + item.quantity, 0);
        
        badgeElement.textContent = totalQuantity;
        
        // Show or hide the badge based on total quantity
        if (totalQuantity === 0) {
            badgeElement.classList.add("d-none"); // Hide badge if no items
        } else {
            badgeElement.classList.remove("d-none"); // Show badge if there are items
        }
    }
    ...
    // Add event listeners to all basket buttons
    document.querySelectorAll(".BaskterBtn").forEach((btn) => {
        btn.addEventListener('click', function() {
            let demo = btn.parentElement; 
    
            // Create basket item object
            let basketObj = {
            ...
            const existingItemIndex = addedItemsToBasket.findIndex(item => item.kategoryOFcomp === basketObj.kategoryOFcomp);
    
            if (existingItemIndex >= 0) {
                // If it exists, increase the quantity
                addedItemsToBasket[existingItemIndex].quantity += 1;
            } else {
